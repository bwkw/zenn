...
# TL;DR
- API設計は、アプリケーションの機能性と保守性に深く関わり、非常に重要である
- 良いAPIは、利用者にとって実行可能であり、表現力があり、かつシンプルで予測可能なインターフェースを提供する必要がある
- カスタムメソッド、ロングランオペレーション、再実行可能ジョブ、シングルトンサブリソースなどのデザインパターンを駆使して、複雑な要件に対応することが可能である
...
# 馴染みのあるAPI設計
以下の記事がとてもよくまとまっているのでこちらを参照してください（丸投げw）
...
APIは、利用者にとって実行可能であり、表現力があり、かつシンプルで予測可能なインターフェースを提供することが求められます。単純な標準メソッドを使用するだけでは、時には限界を感じることもありますが、紹介した原則やパターンを駆使し、より快適なAPI生活を送りましょう👋
