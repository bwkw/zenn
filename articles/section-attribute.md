...
ここで、改めて詳細画面を観察してみます。従業員プロフィールも、拠点情報も、導入機器も、結局は「セクション（カード）の中にアトリビュート（フィールド）が並んでいる」という同じ構造をしています。
...
`sectionKey` や `attributeKey` は、この Graph の構造を元に定義しています。つまり、UI に表示されるセクションや属性は、Graph というデータ構造と一致しています。
