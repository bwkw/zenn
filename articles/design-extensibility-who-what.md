...
**主体**は、その拡張の責任を誰が持つかで 3 つに分けています。
...
- `※`: [Template Method](#template-method) は厳密には「サブクラスがステップを埋める」形ですが、本記事では「interface はそのまま、中身の実装を入れ替える」を広義の「実装の差し替え」として同じ列に置いています。詳細は節内を参照してください。
- `-`: 設計として不可能という意味ではなく、軸の定義上「拡張点として外に開いていない」組み合わせ。第三者の行は開くほど公開 API と互換維持のコストが重くなるため、まず Plugin の境界で引くのが現実的だと考え、Plugin 以外は `-` にしています。
...
なお、以下の `createLogger(env)` は、GoF の Factory Method（サブクラスが生成対象を決める形）そのものというより、生成判断を呼び出し側から分離する実務上の factory に寄せた例です。Abstract Factory のコード例は GoF の説明に近い形になっています。
...
私たちのプロダクトでも、行政申請の手続き種別ごとに、識別子・帳票 ID・XML ビルダの生成関数などを 1 つのレジストリに登録しておき、種別を渡すだけで必要な情報一式を引けるようにしています。新しい手続きを足すときに触るのは、レジストリの登録行と、対応する実装クラスだけです。
