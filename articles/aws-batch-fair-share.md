...
そこで、最終的に選んだのは、ジョブキューは 1 つのまま AWS Batch の **Fair Share Scheduling Policy** を適用するアプローチです。

キューは 1 つのままなのでクォータの問題がなく、組織が増えてもインフラの変更は不要です。リソースはプールとして共有されるため、ある組織のアイドル分を別の組織が使えてコスト効率も高い。そして何より、組織間のリソース分配をスケジューラー側で自動的に調整してくれるため、アプリケーション側で「誰をどれだけ優先するか」を管理する必要がありません。

# AWS Batch Fair Share の仕組み

Fair Share Scheduling は、**`shareIdentifier` ごとの累積リソース使用量をもとに、使用量が少ない share のジョブを優先する**スケジューリングポリシーです。先着順ではなく、「最近あまりリソースを使っていない share を優先する」という発想です。
...
| パラメータ           | 意味                                                                                                                               | 設定値             | 理由                                                                                                                                                                                           |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `shareDecay`         | 過去の使用量が「忘れられる」までの時間。この期間の累積使用量をもとに次のジョブ優先度を決める                                       | 3600 秒（1 時間）  | バースト後の優先度を長めに下げたかった。データ連携はリアルタイム性不要のため、1 時間のペナルティ期間は許容範囲。短い値（例：5 分）だとバースト直後にリセットされ、同じ組織が再占有しやすくなる |
| `computeReservation` | まだジョブを持っていない組織のために予約するリソースの割合（%）。予約比率は `(computeReservation / 100) ^ ActiveFairShares` で計算 | 10                 | 1 組織が独占しているとき 10%（= 1 vCPU）を新規組織用に確保。ActiveFairShares が増えると予約は指数的に小さくなるため、複数組織がアクティブな状況ではリソース効率を損なわない                    |
| `weightFactor`       | 組織ごとの配分の重み。値が小さいほど優先度が高く、より多くのリソースが割り当てられる                                               | 全組織 1.0（暗黙） | 当面は全組織同一優先度。将来プランに応じて差をつける場合は、優先度の高いプランに小さい値（例：gold=0.5、silver=0.75、bronze=1.0）を設定する                                                    |
...
つまり、**「まだキューに入っていない組織が現れたときのために、少しだけスロットを空けておく」** というのが `computeReservation` の役割です。複数の組織がすでにアクティブであれば予約はほぼゼロになり、リソースの無駄が生じません。
