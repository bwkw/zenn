...
このプロダクトでは、各国の拠点が業務データを CSV でアップロードします。日本、中国、タイ、ベトナム、インドネシア、フランス、ドイツ——拠点ごとにローカルエンコーディングがバラバラな CSV が飛んできますが、内部では UTF-8 に統一して扱いたい。そこで、アップロードされた CSV のエンコーディングを自動検出して UTF-8 に変換する仕組みが必要でした。
