...
- **シングルバイトエンコーディング（SBCS）**：エンコーディングと言語の組み合わせごとに、よく出現する **3 バイト連接（trigram）のリスト** を持ち、入力バイト列中の trigram がどれだけヒットするかの割合でスコアリング
- **マルチバイトエンコーディング**：バイト列の並び順が正しいパターンかを検証し、さらに **よく使われる文字のリスト（commonChars）** と照合する 2 段構え
...
こうした仕組みのおかげで、**MBCS にはかなり強い**です。

EUC-KR や GB18030、Big5 などは、高い confidence で安定して検出できます。マルチバイトエンコーディングは「特定の範囲のバイトしか出ない」「バイト列のパターンがはっきりしている」といった構造的な特徴が強く、不正なバイト列があれば即座に候補から除外できるうえ、commonChars 照合でも言語間の差がはっきり出やすいのだと思います。
