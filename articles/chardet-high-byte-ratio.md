...
:::message
なお、node-chardet による検出も Heuristic もデコード検証も、すべて**先頭 64KB のサンプル**に対してのみ行っています。CSV は行の繰り返し構造なので、先頭 64KB で統計的特徴は十分に現れますし、残りはストリーミングで処理するため、数 GB の CSV でもパフォーマンスに影響しません。
:::
