...
DRESS CODE では、日本、タイ、ベトナム、インドネシア、シンガポールなど各国の拠点から業務データが CSV でアップロードされます。エンコーディングは拠点ごとにバラバラなため、ICU プロジェクトの文字検出アルゴリズムをベースにした [node-chardet](https://github.com/runk/node-chardet) でエンコーディングを検出し、[iconv-lite](https://github.com/ashtuchkin/iconv-lite) で UTF-8 に変換する仕組みを入れました。
...
- **シングルバイトエンコーディング（SBCS）**
  エンコーディングと言語の組み合わせごとに、よく出現する **3 バイト連接（trigram）のリスト** を持っている。入力バイト列から trigram を抽出し、リストにどれだけ含まれるか（ヒット率）でスコアリングする。
- **マルチバイトエンコーディング（MBCS）**
  まず、バイト列が各エンコーディングのバイト配置ルールに従っているかを検証し、不正な並びがあれば候補から除外する。次に、デコードされた文字が **よく使われる文字のリスト（commonChars）** にどれだけ含まれるかを照合する。

いずれの場合も、もっともスコアが高かったものを `encoding` として、そのときのスコアを `confidence` として返します。

https://unicode-org.github.io/icu/userguide/conversion/detection.html

こうした仕組みのおかげで、node-chardet は **MBCS の検出にはかなり強い** です。

EUC-KR や GB18030、Big5 などは、高い confidence で安定して検出できます。マルチバイトエンコーディングは「特定の範囲のバイトしか出ない」「バイト列のパターンがはっきりしている」といった構造的な特徴が強く、不正なバイト列があれば即座に候補から除外できるうえ、commonChars 照合でも言語間の差がはっきり出やすいためです。

一方で、**SBCS の検出はそうはいきません**。
...
実際に私たちの環境では、**タイ語（TIS-620）→ ギリシャ語（ISO-8859-7）、ベトナム語（Windows-1258）→ 西ヨーロッパ言語（ISO-8859-1）という誤検出が多発** しました。これは node-chardet のバグではなく、TIS-620 / Windows-1258 が検出対象外であること、そしてサポート済みの SBCS 同士でも同じ 1 バイト空間を共有しており trigram パターンの差が小さいという、方式そのものの限界です。

そのため、「node-chardet を入れたから終わり」ではなく、「特に 1 バイト系は誤検出を前提に、何かしらガードを足す必要がある」というのが、この時点での結論でした。

とはいえ、現在のタイやベトナムでは UTF-8 が主流なので、実際に TIS-620 や Windows-1258 の CSV が来るケースはほぼありません。ただ、レガシーなシステムからのエクスポートなど、いつ来るか分からない非 UTF-8 の CSV には備えておく必要があります。
...
たとえば、タイ語 CSV の 1 行は次のようになります。

```
CSV:    สมชาย,ใจดี,25,กรุงเทพ
バイト: CA C1 AA D2 C2 2C E3 A8 B4 D5 2C 32 35 2C A1 C3 D8 A7 E0 B7 BE
        ^^^^^^^^^^^^^^^^^ 2C ^^^^^^^^^ 2C ^^^^ 2C ^^^^^^^^^^^^^^^^^^^^^
          タイ文字(≥0x80)   ,  タイ文字   , 数字  ,      タイ文字
```

カンマ（`0x2C`）と数字（`0x32`, `0x35`）以外はほぼすべて 0x80 以上です。
...
一方、ベトナム語（Windows-1258）はラテン文字ベースです。文字列の大部分は ASCII 範囲のラテン文字で、声調記号付きの文字（ơ, ư, ă, đ など）だけが 0x80 以上を使います。

```
CSV:    Nguyễn,Hương,25,Hà Nội
バイト: 4E 67 75 79 EA DE 6E 2C 48 FD F5 6E 67 2C 32 35 2C 48 E0 20 4E F4 F2 69
        ^^^^^^^^^^^^^^^^^^^^^ 2C ^^^^^^^^^^^^^^ 2C ^^^^ 2C ^^^^^^^^^^^^^^^^^^^^^^
         ê,combining tilde だけ ≥0x80  ư,ơ だけ ≥0x80  数字     à,ô,dot below だけ ≥0x80
```

`Nguyễn` の 7 バイト中、0x80 以上は `EA`（ê）と `DE`（combining tilde）の 2 バイトだけ。残りはすべて ASCII です。
...
この差は実際にどの程度か。各言語でリアルな CSV を 1,000 行生成し、テキストカラムと数値カラムの比率を変えて計測しました。
...
:::message
これは模擬データによる計測値なので、実際の CSV の内容（声調記号の密度や列構成）によって数ポイント程度のズレはあり得ます。また、数値カラムが大半を占める極端な CSV では両方とも比率が下がり、この Heuristic だけでは区別できなくなります。その場合は次のセクションのデコード検証が安全装置として機能します。
:::

ベトナム語は子音がほぼ ASCII で、high byte になるのは đ と一部の母音（ơ, ư, ă 等）、声調記号の combining mark だけです。そのため、声調記号が多い CSV でも Windows-1258 の high byte ratio は 30% 未満に収まります。一方、TIS-620 は典型的な構成（テキスト 3 : 数値 1）でも約 77% あり、両者の間には常に 50 ポイント以上の差があります。

この差を利用して、閾値を中間にあたる **50%** に設定しました。high byte ratio が 50% を超えていたら「タイ語（TIS-620）候補」、50% 以下なら「ベトナム語（Windows-1258）候補」として扱います。

コードにすると、バイト列を 1 回走査して 0x80 以上の割合を返すだけです。

尚、バイト列からエンコーディングを推測する処理はファイルサイズとのトレードオフになりがちですが、今回の実装では node-chardet による検出も Heuristic もデコード検証も、すべて **先頭 64KB のサンプル**に対してのみ行います。CSV は行の繰り返し構造なので、先頭 64KB で統計的特徴は十分に現れますし、残りはストリーミングで処理するため、数 GB の CSV でもパフォーマンスに影響しません。
...
ただし、これはあくまで候補を絞る段階です。「タイ語っぽい」と判定しても、本当にタイ語かどうかはまだ分かりません。

# デコード検証で確定する

high byte ratio で絞った候補が本当に正しいかを、実際にデコードして検証します。候補のエンコーディングでサンプルをデコードし、以下の 2 つの条件を両方満たした場合だけ採用します。

1. **言語固有文字が 1 文字以上含まれている** — タイ語なら Unicode の Thai ブロック（U+0E00〜U+0E7F）、ベトナム語なら ơ, ư, ă, đ, â, ê, ô といったラテン拡張文字
...
node-chardet はマルチバイト系の検出には強い一方で、TIS-620 や Windows-1258 は検出対象外であり、サポート済みの SBCS 同士でも構造的に区別が難しいという限界があります。これはライブラリのバグではなく、検出対象の範囲と、シングルバイトエンコーディングという仕組みの本質的な制約です。
...
ただし、この手法はファイル全体が単一のエンコーディングであることが前提です。1つの CSV 内に複数のエンコーディングが混在している場合（例：一部の列だけ CP932 で残りが UTF-8）は、自動検出では原理的に対処できません。それでも、「単一エンコーディングの SBCS を正しく検出する」という範囲では、十分に実用的なガードになっています。

この記事が、エンコーディング自動検出で同じような壁にぶつかっている方の参考になれば幸いです👋
