...
node-chardet はマルチバイト系には強い一方で、TIS-620 や Windows-1258 はそもそも検出対象外であり、サポート済みの SBCS 同士でも構造的に区別しにくいという課題があります。これはライブラリのバグではなく、検出対象の範囲と、シングルバイトエンコーディングという仕組みの本質的な限界です。
...
エンコーディング自動検出で同じような壁にぶつかっている方の参考になれば幸いです👋
