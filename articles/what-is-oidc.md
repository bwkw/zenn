...
最近弊チームでは、ネットの記事や認証認可の世界ではお馴染みの [Auth屋](https://authya.booth.pm/) さんの本を読みつつ、適宜議論を交えた認証・認可の勉強会を実施しました。

先日、この勉強会に際し、弊チームのかにさんが **OAuth** についての記事を公開しました。
https://zenn.dev/levtech/articles/a6e8910df5baa0

こちらの記事、**認証・認可の基礎**から **OAuth** までとても分かりやすく書かれているので、まだ読まれていない方は是非読んでみてください！

今回はこの記事に続いて、 **OIDC** について基礎から実践まで整理していきたいと思うので、最後までお付き合いいただけると幸いです。
...
例えば、Google ログイン機能を作成する際に OIDC を利用します。OIDC を利用することで、Google や Facebook などの認証プロバイダを介して安全にユーザーを認証し、プロフィール情報を取得できます。
...
しかし正確には「**一部の OAuth 認可フローを認証に使うとセキュリティーホールが発生するため、認証に使ってはいけない**」というのが正しい理解になります。
...
![認可コードフロー](/images/what_is_oidc/authorization_code_flow.png)

### 認可コードの取得
1. **OIDC 開始**
   エンドユーザーが「Google アカウントでログイン」ボタンを押下することにより、OIDC が開始します。
   リライング・パーティーは OIDC 開始のリクエストを受け付けると、HTTP 302 を返却し、ID プロバイダが提供する**認可エンドポイント**へリダイレクトさせます。
...
| redirect_uri  | 認証が成功した後にユーザーがリダイレクトされるURI。                                       |
...
認証リクエストに含まれる `response_type` の値によって、認証レスポンスに含まれるトークンが変化します。 response_type とフローの対応関係は以下です。
<br>
...
2. **ユーザー認証**
   リダイレクトされたエンドユーザーは、IDプロバイダの認証エンドポイントにアクセスします。ここで、ユーザーは自身の認証情報（例：ユーザー名とパスワード、または多要素認証）を入力して認証を行います。認証が成功すると、IDプロバイダはユーザーに対して承認画面を表示し、リライング・パーティーが要求している情報とアクセス権を確認させます。

3. **ユーザー情報提供に同意**
   ユーザーが承認画面でリライング・パーティーが要求する情報とアクセス権に同意すると、IDプロバイダは認可コードを生成し、ユーザーをリライング・パーティーの指定したredirect_uriにリダイレクトします。この際、リダイレクト先のURLには以下のようなクエリパラメータが付与されます。
...
### UserInfo エンドポイント
OpenID Connect (OIDC) において、クライアントアプリケーションはユーザーのプロフィール情報や認証情報を取得するために、UserInfo エンドポイントにアクセスします。

#### リクエストの流れ
1. **アクセストークンの取得**
クライアントアプリケーションは、認可サーバーが発行する特定のリソースにアクセスする権限を持つアクセストークンを取得します。

2. **UserInfo エンドポイントへのリクエスト**
クライアントアプリケーションは、取得したアクセストークンを使用して UserInfo エンドポイントにリクエストを送信します。このリクエストは一般的に HTTP GET メソッドを使用します。
...
3. **ユーザー情報の取得**
認可サーバーは、アクセストークンの有効性を確認し、有効であればユーザー情報を JSON 形式で返します。
