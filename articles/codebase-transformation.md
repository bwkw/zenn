...
まずは、これらのパターンを [ts-morph](https://github.com/dsherret/ts-morph) を使って AST で検出し、新言語のフィールドに `__TRANSLATE__` という翻訳用マーカーを挿入します。ts-morph は TypeScript の AST を自在に操作できるライブラリです。
...
このように工程を、「抽出」「翻訳」「適用」に分割して進めることで、実装には4時間、実行時間は2時間という投資で大幅な時間削減を実現できました！
