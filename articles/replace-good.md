...
今回リプレイス対象となったのは、CakePHP × jQuery で構築されたレガシーなWebシステムで、このシステムを、NestJS × Next.js を用いたモダンな構成へと置き換えることにしました。
...
では、前置きはこの辺で。ここからは、そんな現場で得た具体的な取り組みや工夫についてご紹介していきます。
...
まずアクセスログをもとに、過去半年間一度も利用されていない API を洗い出しました。さらに、その API が本当に呼び出されていないかを確認するため、対象 API に追加でログを仕込み、2ヶ月間ログ出力がないことをチェックした上で削除を進めました（我々のサービスは1ヶ月で1サイクル機能が利用されるため、2サイクル未使用なら問題なしと判断）。また、リプレイスが完了した箇所についても同様の手法で古いコードを速やかに削除し、現段階で約30本のAPIと1,500行のコードを削除しています（まだ全ては削除しきれていませんが…🥺）。
...
さらに重要なのは、「**プロセスをチーム全員で改善し続ける仕組み**」を作ったことです。ページ単位でのリプレイス完了後の振り返り会において、随時テンプレートを更新しています。これにより、標準プロセスが形骸化せず、常に現場の実態に即した最適な方法論を維持できています。

「プロセスの標準化」は単なるルール作りではなく、「**全員が参加する継続的改善**」そのものだと実感しています。これこそが、リプレイスをスピーディーかつ確実に進める原動力になりました。
...
まず内部品質については、[SonarQube for IDE](https://www.sonarsource.com/jp/products/sonarlint/) の「指摘事項がゼロであること」をを最低限の基準にしています。SonarQube for IDE は、IDE 上でリアルタイムにコード品質をチェックできるので、バグやセキュリティリスク、コードスメルをその場で検出・修正でき、日々のコーディング段階から品質を高めるのにとても役立っています。もともとリプレイス前のコードベースでは、SonarQube で「Maintainability（保守性）」の Severity（深刻度）が Blocker や High（[参考](https://docs.sonarsource.com/sonarqube-server/10.8/instance-administration/analysis-functions/instance-mode/mqr-mode/#mqr-severity)）の指摘が合計で5000件ほどありましたが、現状ではそれを0件まで抑えることができています（もちろん、リプレイスはまだ全て完了したわけではありませんが…）。
