...
AWS Configは、AWSリソースの設定変更を継続的に監視、記録し、セキュリティとコンプライアンスのガイドラインに基づいて評価するサービスです。このサービスは、AWS環境の全体的なセキュリティポスチャを強化し、コンプライアンスの監視と評価を支援します。
...
セキュリティを意識していかなければならないフェーズに直面した際、どこから手を付ければ良いか迷うことがあります。十分なコストをかければセキュリティは強化される可能性がありますが、そのためには経営層の理解と支援が必要です。予防的統制と発見的統制の観点から、出来る部分から、セキュリティリスクの管理と対応を段階的に強化していくことが何よりも重要だと思います。具体的な対応方法については、以下の記事が非常に参考になりますので、是非ご覧ください。
